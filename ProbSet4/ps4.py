...
def build_coder(shift):
    ...
    coder = {}
    for c in alpha_dict:
        coder[c] = letters[(alpha_dict[c] + shift)%52]
    return coder
...
def find_best_shift(wordlist, text):
    ...
    for i in range(0, 53):
        ...
        shifted = apply_coder(text, decoder)
        for w in shifted.split():
            if is_word(wordlist, w) is True:
                counter += 1
        if counter > maxVal[1]:
            maxVal = [i, counter]
    return maxVal
...
def apply_shifts(text, shifts):
    ...
    for t in shifts:
##        if t[0] <= len(text):
          text = text[:t[0]] + apply_shift(text[t[0]:], t[1])
          print text
    print text
...
def find_best_shifts(wordlist, text):
    ...
    while start < len(text):
        ...
        if type(recursive) == list:
            ...
            shifted += 1
            if shifted > 53:
                print "More than 53 shifts."
            text = text[:start] + apply_shift(text[start:], 1)
    print shifts
...
def find_best_shifts_rec(wordlist, text, start):
    ...
    if is_word(wordlist, temp[0]) is False:
        return False
    else:
        print temp[0]
        return [start, len(temp[0])]
