...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    """
    k_results = {}
    split = randomPartition(points, .8)
    training, holdout = split[0], split[1]
    for k in kvals:
        clusters, empty = kmeans(training, k, cutoff, County, minIters = 3, maxIters = 100)
        t_error = 0.0
        for c in clusters:
            for p in c.points:
                dist = p.distance(c.centroid)
                t_error += (dist)**2
        h_error = 0.0
        for p in holdout:
            minDist = (p.distance(clusters[0].centroid))**2
            for c in clusters[1:]:
                dist = (p.distance(c.centroid))**2
                if dist < minDist:
                    minDist = dist
            h_error += minDist
        k_results[k] = [t_error, h_error]
    t_errors, h_errors = [], []
    return k_results

##k_results = graphRemovedErr(points)
##
##k_ratios = {}
##for k in k_results.keys():
##	k_ratios[k] = k_results[k][0]/k_results[k][1]
##ratios = []
##for k in kvals:
##    ratios.append(k_ratios[k])
##
##t_errors, h_errors = [], []
##for k in kvals:
##    t_errors.append(k_results[k][0])
##    h_errors.append(k_results[k][1])
##
##pylab.plot(kvals, t_errors, 'r-')
##pylab.plot(kvals, h_errors, 'g-')
##pylab.title('Removed error from training sets and holdout sets')
##pylab.ylabel('Total error')
##pylab.xlabel('Number of clusters')
##pylab.show()
##
##pylab.plot(kvals, ratios)
##pylab.title('Ratio of removed error of training sets to holdout sets')
##pylab.ylabel('Ratio of training set error to holdout set error')
##pylab.xlabel('Number of clusters')
##pylab.show()
...
def attr_recon(points):
    'cycles through each attribute, determining min, max, median, std_dev'
    'returns a dictionary with min, max, median, std_dev'

def mod_attrs(points):
    'generate arrays from all attrs; find median and std'
    'create lists of median, median/std; convert to array'
    'normal-attrs /= std, -= medstd'
    'run shit'
