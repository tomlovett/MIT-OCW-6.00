...
def get_word_score(word, n):
    ...
    if len(word) == 0:
        return points
    for c in word:
        points += score_values[c] * len(word)
    if len(word) == n:
        points += 50
    return points
...
def is_valid_word(word, hand, word_list):
    ...
    for c in word:
        if hand.has_key(c) is False:
            return False
        if word.count(c) > hand[c]:
            return False
    return word_list.__contains__(word)
...
# 
def play_game_user(word_list, n = 6, hand = {}):
    """
    ...
    * If the user inputs anything else, ask them again.
    ...
    if hand == {}:
        hand = deal_hand(n)
    c = (raw_input("(N)ew game, (r)epeat the previous hand or (e)xit game? - ")).lower()
    if c == 'n':
        print
        hand = deal_hand(n)
    ...
    if c == 'e':
        ...
        print "Let's just try that one again..."
        play_game(n, hand)
    print 
